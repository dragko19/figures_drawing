...
void RectTrafoTest()
{
    ...
    for (const auto& str : rectTest)
    {
        ...
        {
            cout << ex.what() << " figure: unexpected exception in >>\n";
        }
        cout << endl;
    }
}
...
int main()
{
    ...
    {
        vector<FPoint> first = {{10,10}, {20,20}, {-10,-10}};
        vector<FPoint> second = {{20,20}, {10,10}, {10,10}};
        vector<bool> ans = {true, false, true};

        for(int i = 0; i < (int)first.size(); i++)
        {
            if((first[i] < second[i]) != ans[i])
               cout << "Error in FPoint operator<" << endl;
        }
    }
